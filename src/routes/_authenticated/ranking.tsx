...
function RankingPage() {
  ...
  const [data, setData] = useState<{ id: string; nome: string; ativo: number; total: number; pct: number }[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      (rows ?? []).forEach((r: any) => {
        ...
        if (!map.has(r.usuario_id)) map.set(r.usuario_id, { nome: r.profiles?.nome ?? "—", ativo: 0, total: 0 });
        const bucket = map.get(r.usuario_id)!;
        bucket.total += dur;
        if (r.status === "ATIVO") bucket.ativo += dur;
      });
      const arr = Array.from(map.entries()).map(([id, v]) => ({
        id, ...v, pct: v.total > 0 ? (v.ativo / v.total) * 100 : 0,
                  ...
                  {data.map((u, i) => {
                    ...
                    return (
                      <li key={u.id} className="flex items-center gap-4 py-4">
                        <div className={`grid h-10 w-10 place-items-center rounded-full bg-muted ${colors[i] ?? "text-muted-foreground"}`}>
                          {i < 3 ? <Icon className="h-5 w-5" /> : <span className="text-sm font-semibold">{i + 1}</span>}
                        </div>
                        <div className="flex-1">
                          ...
                          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted">
                            <div className="h-full bg-primary" style={{ width: `${Math.min(100, u.pct)}%` }} />
                          </div>
                        </div>
