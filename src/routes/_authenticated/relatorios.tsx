...
function Reports() {
  ...
  const load = async () => {
    ...
    let q = supabase.from("registros_atividade").select("*, profiles(nome)").gte("inicio", fromIso).lte("inicio", toIso).order("inicio", { ascending: false });
  ...
  const exportRows = useMemo(() => buildRows(rows.map((r) => ({
    ...r, usuario: r.profiles?.nome ?? "",
  }))), [rows]);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => exportCSV(exportRows)}><Download className="mr-2 h-4 w-4" />CSV</Button>
            <Button size="sm" variant="outline" onClick={() => exportExcel(exportRows)}><FileSpreadsheet className="mr-2 h-4 w-4" />Excel</Button>
            <Button size="sm" variant="outline" onClick={() => exportPDF(exportRows)}><FileText className="mr-2 h-4 w-4" />PDF</Button>
          </div>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="font-mono text-xs">{formatDateTime(r.inicio)}</TableCell>
                    <TableCell className="font-mono text-xs">{r.fim ? formatDateTime(r.fim) : <span className="text-muted-foreground">em andamento</span>}</TableCell>
