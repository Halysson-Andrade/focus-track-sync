-- Central de Categorização — fonte de dados da tela /categorias (superadmin).
--
-- Agrega, de forma DISTINTA por identificador, todo domínio (navegação) e
-- processo (apps desktop) usados na janela informada, somando o tempo
-- TRABALHADO (duração líquida de ócio, espelhando tempoTrabalhado do front) para
-- ordenar os pendentes por impacto. O GROUP BY garante payload pequeno mesmo com
-- janela longa. O match com `categoria_atividade` (pendente vs já categorizado) é
-- feito no cliente, reusando buildCategoriaIndex — mesma lógica do dashboard.
--
-- Acesso restrito a SUPERADMIN, com visão GLOBAL (todos os usuários). Espelha o
-- padrão SECURITY DEFINER de office_overview/office_detail.
...
BEGIN
  ...
  -- p_dias <= 0 (ou nulo) => "tudo" (sem corte temporal).
