...
--
-- Problema: o ócio é 100% derivado (eventos_ociosidade ∩ janelas ATIVO) e não tem
-- noção de motivo. Reunião presencial, treinamento, atendimento fora do computador
-- ou queda de internet entram como ociosidade e empurram o colaborador para o topo
-- do ranking de /inatividade, sem defesa possível.
--
-- Aqui um INTERVALO ocioso pode ser justificado. O ócio justificado é descontado do
-- número exibido no Espelho de Ponto e do ranking de inatividade — mas NADA do bruto
-- é apagado ou editado: `eventos_ociosidade` continua intacto, exatamente como
-- `ajustes_jornada` é overlay de `registros_atividade` (20260619120000). O desconto
-- é recalculado no cliente (src/lib/ocio.ts), fonte única do ócio.
--
-- Hierarquia (idêntica à de ajustes_jornada):
--   • superadmin  → justifica qualquer colaborador, aplicado direto
--   • admin       → colaborador da MESMA área (same_area), aplicado direto
--   • colaborador → só a si mesmo, entra 'pendente' até a gestão decidir
--
-- Reusa o enum public.ajuste_status ('pendente'|'aprovada'|'rejeitada') em vez de
-- criar outro — menos superfície e zero churn nos tipos gerados.
...
CREATE TABLE IF NOT EXISTS public.justificativas_ociosidade (
  ...
  usuario_nome          text NOT NULL,            -- denorm: sobrevive ao purge dos brutos
  departamento          text,                     -- denorm: recorte por setor no relatório
  dia                   date NOT NULL,            -- dia da JORNADA (critério do espelho)
  inicio                timestamptz NOT NULL,     -- intervalo obrigatório (sem "dia inteiro")
  fim                   timestamptz NOT NULL,
  justificativa         text NOT NULL,            -- motivo da SOLICITAÇÃO
  status                public.ajuste_status NOT NULL DEFAULT 'pendente',
  decidido_por          uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  decidido_por_nome     text,
  justificativa_decisao text,                     -- motivo da DECISÃO (obrigatório na RPC)
  decidido_em           timestamptz,
  criado_por            uuid NOT NULL,            -- quem lançou (≠ usuario_id quando gestor)
  criado_em             timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT justificativas_ociosidade_periodo_chk CHECK (fim > inicio)
);

-- Overlay por usuário/dia (espelho), varredura por período (relatório) e fila de
-- pendentes (menu de aprovação).
...
-- SELECT: o dono, o superadmin (tudo) e o admin da MESMA área — mesma policy de
-- ajustes_jornada_select. Sem policy de INSERT/UPDATE/DELETE: as RPCs são a via.
...
-- ===========================================================================
-- Uma RPC só cobre os dois fluxos. O gate de aprovação direta espelha exatamente o
-- de decidir_justificativa_ociosidade, então a autoridade é a mesma da fila.
...
BEGIN
  ...
  -- Gestor (superadmin/admin) com autoridade sobre o alvo aplica direto — inclui o
  -- gestor justificando a si mesmo. Colaborador comum → pendente.
...
-- ===========================================================================
-- 3) Decisão (aprovar / rejeitar / revogar)
-- ===========================================================================
-- Diferença deliberada vs. decidir_ajuste_jornada: aqui uma justificativa JÁ decidida
-- pode ser re-decidida. Rejeitar uma 'aprovada' = REVOGAR o desconto, devolvendo o
-- ócio ao valor bruto. Isso dispensa RPC de exclusão + tabela de auditoria (como
-- ajustes_jornada_exclusoes): o histórico fica na própria linha, com autor e motivo.
...
-- ===========================================================================
-- Recriada a partir do corpo vigente (20260701150010_e3435e00-...), mudando APENAS o
-- jsonb_build_object (chave 'justificativas'). Assinatura inalterada → sem regeneração
-- de tipos. Devolve TODOS os status: o cliente desconta só 'aprovada' e exibe as
-- pendentes na folha.
...
-- ===========================================================================
-- Recriada a partir do corpo vigente (20260807165409_29df9aac-...), mudando APENAS o
-- jsonb_build_object. Reaproveita o recorte por papel já montado em v_ids, então a
-- justificativa segue exatamente o mesmo escopo do ranking.
