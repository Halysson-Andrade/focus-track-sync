-- Espelho de ponto — acesso por papel (amplia o gate da RPC espelho_ponto).
--
-- Até aqui a função era superadmin-only. Passa a seguir a MESMA hierarquia já
-- usada em ajustes_jornada_select / decidir_ajuste_jornada / office_detail:
--   - superadmin  -> qualquer colaborador
--   - o próprio   -> seu próprio espelho (auth.uid() = p_usuario)
--   - admin       -> colaboradores da MESMA área (same_area)
--
-- Recria a função (CREATE OR REPLACE) preservando o corpo da versão anterior
-- (20260623194950) e alterando SOMENTE o gate. A autorização é do backend; a UI
-- só molda o que cada papel pode pedir. Assinatura inalterada (tipos não mudam).
...
BEGIN
  ...
  END IF;
  -- Hierarquia: superadmin (todos) · o próprio usuário · admin da MESMA área.
  IF NOT (
