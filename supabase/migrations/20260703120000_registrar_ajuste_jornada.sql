-- Lançamento de ajuste de jornada pela GESTÃO, direto da folha (Espelho de Ponto).
--
-- Complementa solicitar_ajuste_jornada (20260619120000_ajustes_jornada.sql), que só
-- cria para o próprio usuário e sempre como 'pendente'. Aqui o solicitante pode ser
-- um gestor lançando o ajuste de um colaborador que ele enxerga:
--   • superadmin  → qualquer colaborador
--   • admin       → colaborador da MESMA área (same_area)
--   • colaborador → só a si mesmo
-- Quando quem lança é gestor (superadmin/admin) com autoridade sobre o alvo, o ajuste
-- já entra 'aprovada' (aplicado direto — mesma UX de editar a jornada no dashboard);
-- colaborador comum sobre si mesmo cai em 'pendente', como antes.
--
-- Continua NÃO-DESTRUTIVO: o tracking bruto (registros_atividade) nunca é alterado; um
-- ajuste 'aprovada' vira overlay recalculado no front. O gate de aprovação direta
-- espelha exatamente o de decidir_ajuste_jornada (superadmin OU admin+same_area), então
-- a autoridade é idêntica à da fila /aprovacoes. Sem policy de INSERT: a RPC é a via.
...
BEGIN
  ...
  -- Lançar para outro colaborador exige autoridade sobre ele.
  IF v_alvo <> v_uid AND NOT v_admin_alvo THEN
    RAISE EXCEPTION 'not allowed to adjust this user';
  END IF;

  -- Aprova direto quando quem lança é gestor (superadmin/admin) com autoridade sobre o
  -- alvo — inclui o gestor ajustando a si mesmo. Colaborador comum → pendente.
